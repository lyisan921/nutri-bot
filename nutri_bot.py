...
def main():
    # Вставь сюда свой токен вместо "YOUR_TOKEN_HERE"
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            GENDER: [
                MessageHandler(filters.Regex("^(м|ж)$", flags=re.IGNORECASE), gender),
            ...
            ACTIVITY: [MessageHandler(filters.Regex("^(низкий|средний|высокий)$", flags=re.IGNORECASE), activity)],
            GOAL: [MessageHandler(filters.Regex("^(похудение|поддержание|набор)$", flags=re.IGNORECASE), goal)],
            TEST_START: [MessageHandler(filters.Regex("^(да|нет)$", flags=re.IGNORECASE), test_start)],
